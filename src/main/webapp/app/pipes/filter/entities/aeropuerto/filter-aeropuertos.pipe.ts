...
@Pipe({
  name: 'filterAeropuertos',
})
export class FilterAeropuertosPipe implements PipeTransform {
    ...
    for (const element of value) {
      ...
      for (const campo of atri) {
        switch (campo) {
          ...
          case 'ciudad': {
            if (element.ciudad !== undefined && element.ciudad !== null) {
              if (args[0][2] !== undefined && args[0][2] !== null && element.ciudad.id.toString().indexOf(args[0][2].toString()) > -1) {
                contArgs++;
              }
            }
            break;
